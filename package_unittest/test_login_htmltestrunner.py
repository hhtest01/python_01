...
#1.用unittest的测试报告太简单，只能在控制台查找，想生成测试报告能更详细，能在浏览器中打开
#2.解决的方案：使用HTMLTestRunner模块生成测试报告。原理：将运行的测试用例结果输出到HTML文件中，就能使用浏览器打开
#前置条件：直接现成的
"""
类：HTMLTestRunner(f,title,descripton)
    f:是指文件对象，一般是HTML
    title：生成的测试报告标题
...
#case3：输入空的用户名或密码登录
    def test_password_is_null(self):
        print("开始运行方法：", sys._getframe().f_code.co_name)
        except_value = 1
        actual_value = login("abc", "").get("code")
        self.assertEqual(except_value, actual_value)
